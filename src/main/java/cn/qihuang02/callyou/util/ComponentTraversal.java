...
public final class ComponentTraversal {
    public static <T> @Nullable T findFirst(
        ...
        while (!stack.isEmpty()) {
            ...
            if (current.getContents() instanceof TranslatableContents) {
                TranslatableContents translatable = (TranslatableContents) current.getContents();
                Object[] args = translatable.getArgs();
                for (int i = args.length - 1; i >= 0; i--) {
                    Object arg = args[i];
                    if (arg instanceof Component) {
                        stack.push((Component) arg);
                    }
                }
            }
        }
